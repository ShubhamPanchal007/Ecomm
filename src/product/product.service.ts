...
export class ProductService {
  ...
  async createProduct(product: PhoneAccessorie): Promise<PhoneAccessorie> {
    // let vendorLocation: any = await this.vendorService.findVendorByID(
    //   product.vendorID,
    // );
    // vendorLocation = vendorLocation.coordinates;
  ...
  async findProductByID(id, productUpdateDto) {
    ...
    const product = await this.PhoneAccessorieModal.findOneAndUpdate(
      ...
      productUpdateDto,
      {
        new: true /*if passed -->,overwrite: true then our patch request would become a put request*/,
      },
    );
    return product;
  }
